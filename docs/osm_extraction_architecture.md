...
Building fitting is mode based. Ordinary rectangular residential-like outlines use `single_rect`: the source polygon is summarized by its minimum rotated rectangle, catalog footprints are compared by physical half-cell or diagonal dimensions, and only the best few footprint types are placed around the source centroid with small local shifts. Large, special, or clearly low-rectangularity outlines use `modular_cover`, which is bounded by piece, state, and time limits and keeps a single-rectangle fallback. Before accepting a candidate, the fitter converts it to the exact building row it would emit and reconstructs the CM footprint through the same final-row profile path used by output validation. Road and other linear overlap checks remain local to candidate windows and use that final-row reconstructed footprint polygon, not just the output anchor, reserved normal cells, or a pre-output approximation; any meaningful polygon overlap with linear occupied geometry rejects the candidate. The `building_footprints` debug layer is also polygon-owned and never falls back to drawing a single anchor cell. Selected building geometry keeps the CM output coordinate and grid kind separate from the blocked 8 m normal cells derived from the final reconstructed footprint polygon. Per-building diagnostics report fit mode, MRR dimensions, rectangularity, preferred orientation, best single-rectangle fit, placement counts, selected footprint units, catalog row/column/direction, centroid shift, area error, road overlap, modular use, fallback reason, blocked normal cells, all overlapped normal cells, and selected Shapely footprint geometry for live debug/export layers.
