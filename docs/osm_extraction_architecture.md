...
7. `output_rows.py` validates layered conflicts, reconstructs final emitted row geometry for building-linear collision checks, emits explicit extent markers, validates road structure according to `road_validation_mode`, and normalizes rows for CM AutoEditor.
...
Linear tile assignment uses the profile catalog's direction, cost, and side-connection signatures. Tuple signatures such as `(2, 3)` are exact connector identities, not sets of allowed side tokens; a shared edge is compatible only when both tiles open toward each other and expose the same normalized signature on that edge. Candidate routes are solved as least-cost compatible tile paths, and finalized `LinearNetworkState` components are solved as compatible tile assignments before placements are emitted, so adjacent road, stream, rail, and fence cells must satisfy metadata compatibility rather than merely sharing a broad north/south/east/west direction set. Adjacent road cells from disjoint known source features are ambiguous in row-only validation and are left to topology/source-aware diagnostics instead of being treated as hard output defects. Cross-family `process_avoidance` is a routing-policy diagnostic, not a tile-catalog gap: stream and fence/wall/hedge routes may keep deliberate non-reserved conflict-hole cells in the route record, but those cells are skipped when reserving `LinearNetworkState` and therefore cannot create road/fence or road/stream intersections. Intersections are anchored only at shared routed topology endpoints with three or more incident directions whose arms continue into the next grid square from the selected intersection cell; one-cell stubs are ignored. If no single legal tile can represent an intersection, anchor selection degrades locally: it preserves the deterministic highest-priority road pair, attaches lower-priority arms as nearby legal T-junctions when available, and drops only unattachable arms with `intersection_fallback_failures` diagnostics. Ordinary bends and intermediate path nodes stay as corner or straight route cells. Cardinal-only catalogs reject diagonal route steps instead of coercing them into north/south or east/west tiles. Tile assignment failures are recorded as `tile_assignment_failures` diagnostics with process, route ID when known, cell, required directions, and reason; impossible component-level side-signature assignments use `no_compatible_tile_component`. In warn mode, failed local route pieces are suppressed before occupancy reservation and output assembly while unrelated finalized placements remain eligible for output; in strict mode, tile assignment failures raise `TileAssignmentError`.

Layered output assembly appends the extent marker, clips rows to `idx_bbox`, and only then runs row, layer-conflict, final geometry, and road validation. This preserves the legacy output contract for fractional diagonal or sub-square placements whose display coordinate can fall just outside the map edge even when their source grid cell was a valid in-bounds candidate. Building placements are stricter before row collapse: they must carry explicit CM output coordinates, and their blocked normal cells are checked against linear cells so a multi-cell footprint cannot hide behind its emitted anchor row. After row assembly, `final_geometry.py` reconstructs each building footprint from the actual emitted `xidx/yidx/menu/cat1/cat2/direction/name` row and profile building catalog metadata, then intersects that reconstructed CM footprint against emitted road, rail, stream, and fence cells. This final-row reconstruction is the authoritative building-linear collision check; the older normal-cell intersection count is retained only as a coarse benchmark diagnostic named `cell_building_linear_collisions`. Row and layer-conflict validation remain strict. Road-structure validation supports `"strict"` and `"warn"` modes: strict raises `OutputRowValidationError`, while warn returns the rows and records `road_validation` plus `road_validation_status` diagnostics with mode, validity, summary, and hard issue count.

Building fitting is mode based. Ordinary rectangular residential-like outlines use `single_rect`: the source polygon is summarized by its minimum rotated rectangle, catalog footprints are compared by physical half-cell or diagonal dimensions, and only the best few footprint types are placed around the source centroid with small local shifts. Large, special, or clearly low-rectangularity outlines use `modular_cover`, which is bounded by piece, state, and time limits and keeps a single-rectangle fallback. Building CSV coordinates follow the legacy OSM extractor convention: the fitter first snaps to a selected placement-grid coordinate on the normal sub-square grid or diagonal grid, emits `xidx - 0.25` and `yidx + 0.25`, and all final footprint checks invert that correction before reconstructing the CM footprint. Before accepting a candidate, the fitter uses the exact building row it would emit and reconstructs the CM footprint through the same selected-grid convention used by output validation. Road and other linear overlap checks remain local to candidate windows and use that final-row reconstructed footprint polygon, not just the output anchor, reserved normal cells, or a pre-output approximation; any meaningful polygon overlap with linear occupied geometry rejects the candidate. The `building_footprints` debug layer is also polygon-owned and never falls back to drawing a single anchor cell. Selected building geometry keeps the selected grid coordinate, emitted CM output coordinate, and grid kind separate from the blocked 8 m normal cells derived from the final reconstructed footprint polygon. Per-building diagnostics report fit mode, MRR dimensions, rectangularity, preferred orientation, best single-rectangle fit, placement counts, selected footprint units, catalog row/column/direction, selected grid coordinates, emitted output coordinates, output roundtrip error, final reconstructed bounds, centroid shift, area error, road overlap, modular use, fallback reason, blocked normal cells, all overlapped normal cells, and selected Shapely footprint geometry for live debug/export layers.
