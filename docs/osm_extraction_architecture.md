...
Building fitting first shortlists catalog footprint types with cheap area, orientation, diagonal, modular, and aspect checks, then generates a bounded set of shifted placements around centroid, oriented-rectangle, and bbox anchors. Only that shortlist reaches the Shapely IoU and road-overlap scoring path, and road overlap is evaluated from local occupied-cell windows rather than by scanning every linear cell. Modular multi-cell candidates are considered when the outline is large enough or independent footprints fit poorly; equivalent top candidates use profile weights for deterministic seeded variation. Per-building diagnostics report compact candidate counts, selected footprint metadata, IoU, centroid shift, and area error; selected Shapely footprint geometry is emitted only when the fitter is constructed with debug geometry enabled.
