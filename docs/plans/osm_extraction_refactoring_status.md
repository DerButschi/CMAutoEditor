...
| 6 | Corridor-limited integer-grid routing | Done | Codex | 2026-05-06 | Added typed route records, heap-based integer-grid routing, occupancy-aware corridor retries, explicit failures, route metrics, and `use_new_network_router` pipeline flag wiring. |
...
| 2026-05-06 | Keep `use_new_network_router` default-off and expose routing through `ExtractionPipeline.run_network_router`. | Milestone 6 proves the bounded router in isolation while tile assignment, output rows, and adapter activation still belong to later milestones. | Codex |
| 2026-05-06 | Route topology edges with shared integer anchors per topology node and cardinal `heapq` A* states `(xidx, yidx, incoming_direction)`. | This removes the full-grid NetworkX hot path from the new router while preserving shared intersection anchors for Milestone 7 tile solving. | Codex |
...
| 2026-05-06 | 6 | `NetworkRouter` is implemented behind `use_new_network_router`, but `OSMProcessor.run_processors` still uses legacy NetworkX routing by default. | Later milestones still need tile catalog assignment, layered output rows, and adapter activation before public extraction can switch to typed routes end to end. | Milestones 7 through 9. |
| 2026-05-06 | 6 | The new router compiles cardinal moves and records route geometry/diagnostics, but catalog-specific transition constraints are intentionally minimal until tile assignment owns concrete tile variants. | Milestone 6 needs the integer route path and no-NetworkX guarantee; Milestone 7 owns per-tile catalog semantics and intersection tile selection. | Milestone 7 tile catalog assignment and intersection solving. |
...
| 2026-05-05 | 4 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\pytest.exe tests\cm_terrain_extractor\osm_extraction\test_area_rasterizer.py -q` | Failed as expected before implementation | TDD red step: `area_rasterizer.py` and `ExtractionPipeline.run_area_rasterizer` did not exist yet; 7 failures. |
| 2026-05-05 | 4 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\pytest.exe tests\cm_terrain_extractor\osm_extraction\test_area_rasterizer.py -v` | Passed | 7 passed. Existing `.pytest_cache` permission warning remains. |
...
| 2026-05-06 | 6 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\pytest.exe tests\cm_terrain_extractor\osm_extraction\test_network_routing.py -q` | Failed as expected before implementation | TDD red step: `network_routing.py` did not exist and `ExtractionPipeline.run_network_router` was not defined. |
| 2026-05-06 | 6 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\pytest.exe tests\cm_terrain_extractor\osm_extraction\test_network_routing.py -q` | Passed | 7 passed. Existing `.pytest_cache` permission warning remains. |
| 2026-05-06 | 6 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\pytest.exe tests\cm_terrain_extractor\osm_extraction\test_network_routing.py -v` | Passed | 7 passed. Existing `.pytest_cache` permission warning remains. |
| 2026-05-06 | 6 | `Select-String -Path cm_terrain_extractor_app\terrain_extraction\osm_extraction\network_routing.py -Pattern "networkx","nx."` | Passed | No matches; the new routing module does not import or reference NetworkX. |
| 2026-05-06 | 6 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\pytest.exe tests\cm_terrain_extractor\osm_extraction\test_models.py tests\cm_terrain_extractor\osm_extraction\test_pipeline.py tests\cm_terrain_extractor\osm_extraction\test_network_topology.py tests\cm_terrain_extractor\osm_extraction\test_network_routing.py -q` | Passed | 19 passed. Existing dependency warnings and `.pytest_cache` permission warning remain. |
| 2026-05-06 | 6 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\pytest.exe tests\cm_terrain_extractor\osm_extraction -q` | Passed | 54 passed. Existing legacy/dependency warnings and `.pytest_cache` permission warning remain. |
| 2026-05-06 | 6 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\ruff.exe check cm_terrain_extractor_app\terrain_extraction\osm_extraction\network_routing.py cm_terrain_extractor_app\terrain_extraction\osm_extraction\models.py cm_terrain_extractor_app\terrain_extraction\osm_extraction\stats.py cm_terrain_extractor_app\terrain_extraction\osm_extraction\pipeline.py cm_terrain_extractor_app\terrain_extraction\osm_extraction\__init__.py tests\cm_terrain_extractor\osm_extraction\test_network_routing.py --no-cache` | Failed during cleanup, then passed | Initial run found import ordering and one unused import; manual cleanup was applied. Final run: all checks passed. |
| 2026-05-06 | 6 | `C:\Users\der_b\miniconda3\envs\cm_terrain\Scripts\ruff.exe check cm_terrain_extractor_app\terrain_extraction\osm_extraction\network_routing.py cm_terrain_extractor_app\terrain_extraction\osm_extraction\stats.py tests\cm_terrain_extractor\osm_extraction --no-cache` | Passed | Plan-listed ruff target passed. |
...
- `use_new_network_router` remains a migration flag and is default-off for app-facing extraction until tile assignment, output assembly, and adapter activation milestones consume typed routes.
