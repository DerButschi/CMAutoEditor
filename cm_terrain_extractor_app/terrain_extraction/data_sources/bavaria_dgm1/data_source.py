...
class BavariaDataSource(GeoTiffDataSource):
    def __init__(self):
        self.name = 'Bavaria DGM1'
        self.data_type = 'geotiff'
        self.model_type = 'DTM'
        self.resolution = '1 m'
        self.country = 'Germany/Bavaria'
        self.crs = CRS.from_epsg(25832)
        self.gdf = None
        self.outline: MultiPolygon = None
        self.data_folder = 'bavaria_dgm1'
        self.current_merged_image_path = None,
        self.cached_data: pandas.DataFrame = None
        self.cached_data_bounding_box: BoundingBox = None
        self.envelope = Polygon([(497999.5, 5234999.5), (855999.5, 5234999.5), (855999.5, 5602999.5), (497999.5, 5602999.5), (497999.5, 5234999.5)])
        self.gdf_geojson_path = 'cm_terrain_extractor_app/terrain_extraction/data_sources/bavaria_dgm1/bavaria_dgm1.geojson'
    ...
    def get_missing_files(self, bounding_box: BoundingBox, data_storage_folder: str) -> List[str]:
        ...
        for _, entry in gdf[gdf.intersects(bounding_box.get_box(self.crs))].iterrows():
            relative_location = os.path.join(self.data_folder, os.path.basename(entry['url']))
    ...
    def get_images_in_bounding_box(self, bounding_box: BoundingBox, outdir):
        ...
        for dir_path, dir_names, file_names in os.walk(os.path.join(outdir, self.data_folder)):
            for file_name in file_names:
                gdf_in_bbox = self.gdf[self.gdf['url'].apply(lambda x: os.path.basename(x)) == file_name]
